...
@router.get("/{empleado_id}", response_model=EmpleadoDTO, dependencies=[Depends(check_role(["ADMIN", "USUARIO"]))])
...
@router.get("/compania/{compania_id}", response_model=PaginatedResponse[EmpleadoDTO], dependencies=[Depends(check_role(["ADMIN", "USUARIO"]))])
...
@router.delete("/lote", status_code=204, dependencies=[Depends(check_role(["ADMIN"]))])
...
@router.delete("/{empleado_id}", status_code=204, dependencies=[Depends(check_role(["ADMIN"]))])
