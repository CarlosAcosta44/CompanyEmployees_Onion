...
def listar_empleados(
    ...
    return service.listar_paginados(pagina, tamano, orden, dir, buscar)
...
def listar_empleados_por_compania(
    compania_id: UUID,
...
def crear_empleados_lote(dtos: list[EmpleadoCreateDTO] = Body(...), service: EmpleadoService = Depends(get_empleado_service)):
    logger.info("[Controller] POST /api/empleados/lote")
    return service.crear_en_lote(dtos)
...
def actualizar_empleado_parcial(empleado_id: UUID, dto: EmpleadoUpdateDTO, service: EmpleadoService = Depends(get_empleado_service)):
    logger.info("[Controller] PATCH /api/empleados/%s", empleado_id)
    return service.actualizar_parcial(empleado_id, dto)


@router.delete("/lote", status_code=204)
def eliminar_empleados_lote(empleado_ids: list[UUID] = Body(...), service: EmpleadoService = Depends(get_empleado_service)):
    logger.info("[Controller] DELETE /api/empleados/lote")
    service.eliminar_en_lote(empleado_ids)
